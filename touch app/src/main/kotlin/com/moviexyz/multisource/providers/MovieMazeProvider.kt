...
class MovieMazeProvider : MainAPI() {
    override var mainUrl = "https://moviemaze.cc"
    override var name = "MovieMaze"
    override val hasQuickSearch = true
    override val supportedTypes = setOf(TvType.Movie, TvType.TvSeries)
    
    override val mainPage = mainPageList {
        add(HomePageList("Featured Movies", listOf()))
        add(HomePageList("Latest Releases", listOf()))
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = document.selectFirst("h1.title, .movie-title")?.text()?.trim() 
            ?: document.title().removeSuffix(" - MovieMaze").trim()
